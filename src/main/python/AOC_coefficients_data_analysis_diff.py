...
def circularity(ya, yb, c, c1=1, kappa=0):
    return ya * c1 #(ya-yb*c)/(ya + yb*c + kappa) #*c1
...
class AOCcoefficients(object):
    def __init__(self):
        ...
        self.bParVary = False # True
        self.ymaxParVary = False # True
        self.bPerVary = False # True
        self.ymaxPerVary = False # True
        self.cVary = True
        self.c1Vary = True
        self.varyProc = 0.005
    ...
    def importExperimentalData(self):
        print('start importExperimentalData')
        self.experiment_colums = [self.BexpName, self.ParExpName, self.PerExpName] #, self.semPar, self.semPer]
        print(self.experiment_colums)
        self.expData1 = pd.read_csv(self.expFilename,  sep='[,;\s+]',  header=0, usecols=self.experiment_colums).dropna()
        ...
        self.setParCoefficients(b=0, ymax=1)
        ...
        self.setPerCoefficients(b=0, ymax=1)
    ...
    def importTheoryData(self, theoryFilenamePath):
        ...
        # theor['par_norm'] = theor['difference_norm'] #(theor['sigma+']) / max_par + self.kappa
        # theor['per_norm'] = theor['difference_norm'] # (theor['sigma-']) / max_per + self.kappa
        # theor['circularity'] = theor['difference_norm'] #(theor['sigma+'] - theor['sigma-']) / (theor['sigma+'] + theor['sigma-'] + 2 * self.kappa * max_par)
